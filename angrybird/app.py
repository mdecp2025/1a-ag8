...
SECRET_KEY = os.environ.get(
    "SECRET_KEY", "a_very_secret_and_long_key_for_flask_session_security"
)
...
class Score(BaseModel):
    ...
    class Meta:
        indexes = ((("score_value", "timestamp"), False),)
...
class RegistrationForm(Form):
    username = StringField(
        "使用者名稱",
        [validators.Length(min=4, max=25, message="長度必須介於 4 到 25 個字元")],
    )
...
class LoginForm(Form):
    username = StringField(
        "使用者名稱", [validators.DataRequired(message="使用者名稱為必填項")]
    )
...
def index():
    try:
        ...
        top_scores = (
            Score.select(Score.score_value, User.username)
            .join(User)
        ...
        leaderboard_data = [
            {"username": s.user.username, "score": s.score_value} for s in top_scores
        ]
    except Exception as e:
...
def submit_score():
    ...
    if not request.is_json:
        return (
            jsonify({"success": False, "message": "Request must be JSON format."}),
            415,
        )
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Invalid or non-integer score value provided.",
                }
            ),
            400,
        )
    ...
    try:
        ...
        if user_id is None:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Authentication failed or session expired (No user_id).",
                    }
                ),
                401,
            )
        ...
        # 直接傳入 user_id 作為外鍵值
        Score.create(user=user_id, score_value=score_value, timestamp=datetime.now())
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Database error occurred. Please log in again.",
                }
            ),
            401,
        )
